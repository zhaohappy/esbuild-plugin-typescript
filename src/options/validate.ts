...
export function validateSourceMap(
  ...
  if (compilerOptions.sourceMap && !outputOptions.sourcemap && !autoSetSourceMap) {
    context.warn(
      `@libmedia/esbuild-plugin-typescript: Rollup 'sourcemap' option must be set to generate source maps.`
    );
  } else if (!compilerOptions.sourceMap && outputOptions.sourcemap) {
    context.warn(
      `@libmedia/esbuild-plugin-typescript: Typescript 'sourceMap' compiler option must be set to generate source maps.`
    );
  }
}
...
export function validatePaths(
  ...
  if (compilerOptions.out) {
    context.error(
      `@libmedia/esbuild-plugin-typescript: Deprecated Typescript compiler option 'out' is not supported. Use 'outDir' instead.`
    );
  } else if (compilerOptions.outFile) {
    context.error(
      `@libmedia/esbuild-plugin-typescript: Typescript compiler option 'outFile' is not supported. Use 'outDir' instead.`
    );
  }
    ...
    if (compilerOptions[dirProperty] && outputDir) {
      ...
      if (outputOptions.dir) {
        ...
        if (fromRollupDirToTs.startsWith('..')) {
          context.error(
            `@libmedia/esbuild-plugin-typescript: Path of Typescript compiler option '${dirProperty}' must be located inside Rollup 'dir' option.`
          );
        }
        ...
        if (fromTsDirToRollup.startsWith('..')) {
          context.error(
            `@libmedia/esbuild-plugin-typescript: Path of Typescript compiler option '${dirProperty}' must be located inside the same directory as the Rollup 'file' option.`
          );
        }
      } else {
  ...
  if (compilerOptions.declaration || compilerOptions.declarationMap || compilerOptions.composite) {
    if (DIRECTORY_PROPS.every((dirProperty) => !compilerOptions[dirProperty])) {
      context.error(
        `@libmedia/esbuild-plugin-typescript: You are using one of Typescript's compiler options 'declaration', 'declarationMap' or 'composite'. ` +
